...
import (
	...
	"github.com/hurisheng/go-futu-api/protobuf/qotupdateticker"
)

// QotSub 订阅或者反订阅，失败返回Error错误信息
func (api *FutuAPI) QotSub(ctx context.Context, req *qotsub.C2S) (*qotsub.S2C, error) {
	ch := qotsub.NewResponseChan()
	if err := api.send(ProtoIDQotSub, &qotsub.Request{C2S: req}, ch); err != nil {
		return nil, err
	}
	return ch.Response()
}
...
// QotUpdateTicker 实时逐笔回调，异步处理已订阅股票的实时逐笔推送
func (api *FutuAPI) QotUpdateTicker() (<-chan *qotupdateticker.Response, error) {
	ch := qotupdateticker.NewResponseChan()
	if err := api.subscribe(ProtoIDQotUpdateTicker, ch); err != nil {
		return nil, err
	}
